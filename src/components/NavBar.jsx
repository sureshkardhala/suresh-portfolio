  ...
  return (
    ...
    <div className="flex justify-between items-center w-full h-20 text-white bg-black px-4 fixed">
        ...
        <ul className="hidden md:flex ">
            {
                links.map(link =>
                    <li key={link.id} className="px-2 ml-4 cursor-pointer capitalize text-gray-400 font-medium hover:scale-105 duration-200  ">{link.name}</li>
                )
            }
        </ul>
        ...
        </div>
        {
            ...
            {
                links.map(link =>
                    <li key={link.id} className="text-4xl  h-10 px-2  mt-20 cursor-pointer capitalize text-gray-400 font-medium hover:scale-105 duration-200  ">{link.name}</li>
                )
            }
            </ul>
        )}
